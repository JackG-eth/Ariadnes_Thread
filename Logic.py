    ...
    for i in range(comDictLenApp):
        for key, val in officerApp['items'][i].items():
            if(key == 'appointed_to'):
                ...
                # Might be worth removing this if statement (just ensures
                # companies are not duplicated (endless loop))
                if(company_id != company_no):
                    finalDict["Company Name: " + str(i)] = name
                    finalDict["Company ID: " + str(i)] = company_id
                    # maybe comment out this line depending on levels to
                    # prevent exceeding call limit
                    finalDict["InvestmentRisk: " +
                              str(i)] = getCompanyScore(company_id)
                    if(company_no != company_id):
                        if(depth > 1):
                            finalDict["Company ID: " + str(i)] = get_associated_companies_info_by_company(
                                company_id, depth - 1)
                        else:
                            finalDict["Company ID: " + str(i)] = company_id
    return finalDict
...
def get_associated_companies_info_by_company(company_no, depth):
    ...
    # loop through all officers returned store their (name, OfficerID in sub
    # Dictionary)
    ...
    for key, val in storedInfo.items():
        ...
        Final["Officer " + str(officerName)
              ] = get_Officers_Companies(officerID, company_no, depth)
